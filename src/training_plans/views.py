...
from .forms import TrainingPlanForm, WorkoutForm, WorkoutExerciseFormSet, ExerciseSetFormSet
...
class WorkoutFormSetView(View):
    def setup(self, request, *args, **kwargs):
        ...
        self.training_plan = get_object_or_404(TrainingPlan, pk=kwargs.get("training_plan_pk"))
        self.workout = get_object_or_404(Workout, pk=kwargs.get("workout_pk"))

    def get(self):
        workout_exercise_formset = WorkoutExerciseFormSet(instance=self.workout, prefix="exercises")
        exercise_set_formsets = []

        for i, form in enumerate(workout_exercise_formset.forms):
            workout_exercise_instance = form.instance
            formset = ExerciseSetFormSet(instance=workout_exercise_instance, prefix=f"sets-{i}")
        ...
        return render(self.request, "training_plans/workout_exercises_form.html", context)

    def post(self):
        workout_exercise_formset = WorkoutExerciseFormSet(self.request.POST, instance=self.workout, prefix="exercises")
        exercise_set_formsets = []
        ...
        if workout_exercise_formset.is_valid():
            ...
            for i, workout_exercise_form in enumerate(workout_exercise_formset.forms):
                exercise_set_formset = ExerciseSetFormSet(self.request.POST,
                                                          instance=workout_exercise_form.instance,
                                                          prefix=f"sets-{i}")
                exercise_set_formsets.append(exercise_set_formset)

            all_valid = all(exercise_set_formset.is_valid() for exercise_set_formset in exercise_set_formsets)
            if all_valid:
        ...
        context = {
            ...
            "workout": self.workout,
            "workout_exercise_formset": workout_exercise_formset,
            "exercise_set_formsets": list(zip(workout_exercise_formset.forms, exercise_set_formsets)),
        }
